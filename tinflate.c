 ...
 */
// T ADD: Modified to use in wasm, changes marked like this line

//#include "tinf.h" // T ADD: just use defines and skip header file
#define TINF_OK             0
...
// T ADD: change signature and do some magic initialization
static int u(const void *source, unsigned int sourceLen)
{
   tinf_init(); // always init to save few bytes
   void* dest = (void*)65536;
   //unsigned int destLen = 16777216;
   unsigned int destLen = 0xfffff;
   ...
   d.destLen = &destLen; // T ADD: fix for changed type

   //*destLen = 0; // T ADD: do not need it
